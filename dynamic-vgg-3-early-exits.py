...
class BranchyVGG(nn.Module):
    ...
    def _forward_inference(self, x):
        ...
        feature_blocks = [self.features1, self.features2, self.features3]
        exit_blocks = [self.exit1, self.exit2, self.exit3]
...
def train_static_vgg(model, train_loader, test_loader=None, num_epochs=100, learning_rate=0.001, weights_path=None):
    ...
    print("\nStarting Static VGG training with metric tracking...")
...
def train_branchy_vgg(model, train_loader, test_loader, num_epochs=100, learning_rate=0.001, weights_path=None):
    ...
    print("Epoch | Train Loss | Val Acc | Inf Time | Exit Distribution | Combined Score | Best?")
    print("-" * 90)
    ...
    for epoch in range(num_epochs):
        ...
        max_acceptable_time = 50
        norm_time = 1 - min(inference_time / max_acceptable_time, 1)
        early_exit_ratio = (exit_percentages[1] + exit_percentages[2] + exit_percentages[3]) / 100
        ...
        exit_dist_str = f"{exit_percentages[1]:.1f}/{exit_percentages[2]:.1f}/{exit_percentages[3]:.1f}/{exit_percentages[4]:.1f}"
        print(f"{epoch+1:3d}   | {avg_train_loss:.4f}    | {accuracy:5.2f}% | {inference_time:6.2f}ms | {exit_dist_str:15} | {combined_score:.4f}      | {'*' if is_best else ''}")
    ...
    print(f"  Exit 1 (Early): {best_epoch_metrics['exit_distribution'][1]:.1f}%")
    print(f"  Exit 2 (Mid): {best_epoch_metrics['exit_distribution'][2]:.1f}%")
    print(f"  Exit 3 (Late): {best_epoch_metrics['exit_distribution'][3]:.1f}%")
    print(f"  Exit 4 (Final): {best_epoch_metrics['exit_distribution'][4]:.1f}%")
    print(f"Combined Score: {best_epoch_metrics['combined_score']:.4f}")
    if weights_path and best_state_dict is not None:
        torch.save({
            'epoch': best_epoch_metrics['epoch'],
...
def plot_exit_distribution(exit_percentages, dataset_name):
    ...
    bars = plt.bar(
        ['Exit 1 (Early)', 'Exit 2 (Mid)', 'Exit 3 (Late)', 'Exit 4 (Final)'], 
        list(exit_percentages.values()), 
...
def analyze_exit_distribution(model, test_loader, dataset_name):
    ...
    class_distributions = {1: defaultdict(int), 2: defaultdict(int), 3: defaultdict(int), 4: defaultdict(int)}
    total_samples = 0
    with torch.no_grad():
        for images, labels in test_loader:
            images, labels = images.to(device), labels.to(device)
...
def plot_class_distribution(class_distributions, dataset_name):
    ...
    exit_names = ['Early Exit (1)', 'Middle Exit (2)', 'Late Exit (3)', 'Final Classifier (4)']
...
def run_experiments():
    ...
    if os.path.exists(static_weights_path):
        ...
        if static_best_metrics.get('accuracy') is not None:
            print(f"Loaded weights with validation accuracy: {static_best_metrics['accuracy']:.2f}%")
    ...
    print(f"Avg Inference Time: {static_inference_time:.2f} ms")
    ...
    if os.path.exists(branchy_weights_path):
        ...
        if branchy_best_metrics.get('accuracy') is not None:
            print(f"Loaded weights with validation accuracy: {branchy_best_metrics['accuracy']:.2f}%")
    ...
    print(f"Avg Inference Time: {final_inference_time:.2f} ms")
    ...
    print("\nResults Summary:")
    print("\nStatic VGG Results:")
    if static_best_metrics.get('epoch') is not None:
        print(f"Best Epoch (Epoch {static_best_metrics['epoch']}):")
        if static_best_metrics.get('accuracy') is not None:
            print(f"Accuracy: {static_best_metrics['accuracy']:.2f}%")
        if static_best_metrics.get('inference_time') is not None:
            print(f"Inference Time: {static_best_metrics['inference_time']:.2f}ms")
    print(f"\nFinal Test Metrics:")
    print(f"Accuracy: {static_accuracy:.2f}%")
    print(f"Inference Time: {static_inference_time:.2f}ms")
    print(f"Energy: {static_power['energy']:.2f}J")
    print("\nBranchy VGG Results:")
    if branchy_best_metrics.get('epoch') is not None:
        print(f"Best Epoch (Epoch {branchy_best_metrics['epoch']}):")
        if branchy_best_metrics.get('accuracy') is not None:
            print(f"Accuracy: {branchy_best_metrics['accuracy']:.2f}%")
        if branchy_best_metrics.get('inference_time') is not None:
            print(f"Inference Time: {branchy_best_metrics['inference_time']:.2f}ms")
        if branchy_best_metrics.get('exit_distribution') is not None:
            print("Exit Distribution:")
            for exit_num, percentage in branchy_best_metrics['exit_distribution'].items():
                print(f"  Exit {exit_num}: {percentage:.1f}%")
    print(f"\nFinal Test Metrics:")
    print(f"Accuracy: {final_accuracy:.2f}%")
    print(f"Inference Time: {final_inference_time:.2f}ms")
    print(f"Energy: {branchy_power['energy']:.2f}J")
    print("Exit Distribution:")
    for exit_num, percentage in exit_percentages.items():
        print(f"  Exit {exit_num}: {percentage:.1f}%")
    print("\nImprovements:")
