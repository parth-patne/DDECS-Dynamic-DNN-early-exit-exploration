...
class BranchyAlexNet(nn.Module):
    def __init__(self, num_classes=10, in_channels=3):
        ...
        self.exit_loss_weights = [0.10, 0.10, 0.10, 0.70]
        ...
        self.output_pool = nn.AdaptiveAvgPool2d((4, 4))
        self.output_layer = nn.Sequential(
            nn.Dropout(),
    ...
    def _forward_inference(self, x):
        ...
        if len(remaining_indices) > 0:
            x_final = self.output_pool(x_current)
            x_final = x_final.view(x_final.size(0), -1)
    ...
    def _calculate_reward(self, exit_idx, correct):
        ...
        early_exit_bonus = max(0, 3 - exit_idx) * 0.2
...
def get_exit_indices(model):
    ...
    for i in range(1, 10):
        if hasattr(model, f"exit{i}"):
            indices.append(i)
    # Always add the final exit as the next index
    if indices:
...
def plot_comparative_analysis(static_results, branchy_results, dataset_name):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{dataset_name.lower()}_energy_comparison.png'),
                dpi=300, bbox_inches='tight')
    plt.close()

def plot_exit_distribution(exit_percentages, dataset_name):
    output_dir = create_output_directory(dataset_name)
    filtered_exits = {k: v for k, v in exit_percentages.items() if k in [1,2,3,4]}
